...
Event OnHit(ObjectReference akTarget, ObjectReference akAggressor, Form akSource, Projectile akProjectile, bool abPowerAttack, \
  bool abSneakAttack, bool abBashAttack, bool abHitBlocked, string apMaterial)
	...
	if (SceneData.Interrupted <= 0 && SceneIsPlaying)
...
bool Function DoesMainActorPrefID(int seqID)
	...
	return DTSleep_CommonF.IsIntegerInArray(BaseIDForFullSequenceID(seqID), MainActorScenePrefArray)
endFunction

bool Function DoesSecondActorPrefID(int seqID)
	
	return DTSleep_CommonF.IsIntegerInArray(BaseIDForFullSequenceID(seqID), SecondActorScenePrefArray)
endFunction

bool Function DoesSecondActorHateID(int seqID)
	
	return DTSleep_CommonF.IsIntegerInArray(BaseIDForFullSequenceID(seqID), SecondActorSceneHateArray)
endFunction

; return -1 for none, 0=FMM, 1=FMF, 2=M/F
...
bool Function PlayActionDancing()
	...
	elseIf (SleepBedRef && IsObjBed(SleepBedRef))
...
bool Function PlayActionHugs()
	...
	if (SceneData.IsUsingCreature || SceneData.CompanionInPowerArmor || SecondActorRef == None)
		return false
	endIf
		...
		if (SceneData != None && SceneData.MaleRole != None && SceneData.FemaleRole != None)
			...
			if (SceneData.SameGender)
				...
				elseIf (SceneData.MaleRoleGender == 1)
					if (seqID == 181 || seqID == 180 || seqID == 281)
						if (SceneData.HasToyEquipped && SceneData.ToyArmor != None)
							CheckRemoveToys()
						endIf
						return true
...
Function DoDisableControls()
	...
	DTSleepIAQInputLayer.DisablePlayerControls(true, true, true, false, true, false, true, false, true, true, true)
endFunction
...
Function FadeOutSec(float secs, bool doWait = true)
	...
	if (HoldToBlackIsSetOn == false)
		;Debug.Trace(myScriptName + " FadeOutSec - holdBlack")
...
Function FinalizeAndSendFinish(bool seqStartedOK = true, int errCount = 0)
	...
	while (count < 30 && !IsAAFEnabled() && IsSequenceRunning())
		Utility.Wait(0.25)
		count += 1
	endWhile
...
float Function GetTimeForPlayID(int id)
	...
	elseIf (id >= 100)
		...
		elseIf (id == 706 || id == 707 || id == 739 || id == 740 || id == 751)
			DTSleep_IntimateSceneLen.SetValueInt(0)
		...
		elseIf (endurance > 6 && id != 547 && id != 546)
			DTSleep_IntimateSceneLen.SetValueInt(2)
			baseSec += waitSecX2
...
bool Function IsObjSeat(ObjectReference obj)
	...
	if (obj != None)
		...
		elseIf (obj.HasKeyword(AnimFurnBarStoolKY) || obj.HasKeyword(AnimFurnCouchKY))
			return true
...
bool Function PlayIntimateAACWithEndTimer(float timerSecs)
	if (DTSleep_PlayAACSpell != None)
		...
		if (SecondActorRef != None)
			...
			Debug.Trace(MyScriptName + " PlayACC single-caster (not expected to work)") 
...
bool Function PlayIntimateLeitoAnimWithEndTimer(float timerSecs)
	if (DTSleep_PlayLeitoTargetSpell != None)
		if (Debug.GetPlatformName() as bool)
			...
			if (SecondActorRef != None && !SceneData.CompanionInPowerArmor)
				SecondActorRef.PlayIdle(LooseIdleStop2)
				Utility.Wait(0.333)
			...
			while (attemptCount > 0)
				...
				if (id < 1000)
					...
					elseIf (id < 500 && !playerPositioned && !restrictPlaceOnBed && !MainActorPositionByCaller && !mainNodeOnBed)  
						...
						PositionMarkerOnBed(mainNode, SleepBedRef, xOffset, yOffset, 0.0, true, mainActorIsMaleRole, revHeading, useAlternateBedAngle)
				...
				if (SecondActorRef != None)
					...
					if (myMainAtMeNode != None && myMainAtMeNode.Is3DLoaded())
						...
						;elseIf (id > 540 && id < 600)
						;	forceMainActorPosition = true  ; override to force
						...
						elseIf (id >= 700 && id < 760)
							...
							elseIf (id == 754)
								; memory lounge chair -- too close
								if (DTSleep_IntimateChairTooCloseList.HasForm(SleepBedRef.GetBaseObject() as Form))
									markerIsBed = false
									bedUseNodeMarker = true
									yOffset = -4.33
									xOffset = 9.4
							...
							elseIf (id == 778)
								...
								if (zOffset == 0.0)
									...
									;bedUseNodeMarker = true  ; sleepbag or other furniture
									markerIsBed = false
								endIf
							endIf
						...
						if (placeOnBedSimple)
							...
							elseIf (id >= 600 && id < 700)
								...
								if (markerIsBed)
									...
									if (bedUseNodeMarker)
										mainNode.MoveTo(SleepBedRef, xOffset, yOffset, zOffset, true)
										if (headingAngle == 0.0)
...
float Function PositionMarkerOnBedZAdjustForSceneID(int id)
	...
	elseIf (IsObjBed(SleepBedRef) == false)
		...
		if (DTSleep_IntimatePoolTableList.HasForm(SleepBedRef.GetBaseObject() as Form))
	...
	if (DTSleep_PlayerClone != None && hasGameReloaded) ;&& (DTSConditionals as DTSleep_Conditionals).IsF4SE)
		...
		if (MainActorCloneRef != None)
			...
			if (MainActorCloneRef.Is3DLoaded())
				...
				MainActorCloneRef.SetRestrained(true)
				MainActorRef.AddSpell(DTSleep_InvisibiltySpell)
...
int[] Function SceneIDArrayForAnimationSet(int packID, bool mainActorIsMaleRole, bool noLeitoGun, bool standOnly, int[] sidArray, int includeHugs)
	...
	elseIf (packID == 5 && SleepBedRef.HasKeyword(IsSleepFurnitureKY) && !SceneData.CompanionInPowerArmor)   ; Atomic Lust
		; v1.70 - change to limit hugs at beds since player can use chairs
		
		if (includeHugs <= 0 && !SceneData.IsUsingCreature) 

			if (playerPick)
				if (DoesMainActorPrefID(48))
					includeHugs = 1
				endIf				
			endIf
	...
	if (packID > 0 && DTSleep_AdultContentOn.GetValue() >= 2.0 && includeHugs <= 1)
		...
		if (bedIsPillory && !SceneData.IsUsingCreature && !SceneData.CompanionInPowerArmor)
			...
			elseIf (packID == 7 && !SceneData.FemaleRaceHasTail)
				if (!SceneData.SameGender)
					sidArray.Add(51)	; includes 2nd male
				endIf
			endIf
		...
		elseIf (bedIsSeat && SceneData.MaleRole != CompStrongRef)
			...
			if (!SceneData.CompanionInPowerArmor)
				...
				if (packID == 5 && (DTSConditionals as DTSleep_Conditionals).AtomicLustVers >= 2.43 && !SceneData.IsUsingCreature && !SceneData.SameGender)
					if (DTSleep_IntimateChairsList.HasForm(baseBedForm) && SceneData.SecondMaleRole == None && SceneData.SecondFemaleRole == None)
						sidArray.Add(38)
					endIf
				...
				elseIf (packID == 7)
					...
					elseIf (DTSleep_IntimateChairsList.HasForm(baseBedForm))
						...
						elseIf (SceneData.MaleRoleGender == 1)
							sidArray.Add(39)
						endIf
					elseIf (SleepBedRef.HasKeyword(AnimFurnBarStoolKY))
		...
		elseIf (bedIsTable)
			...
			elseIf (!SceneData.IsUsingCreature && !SceneData.CompanionInPowerArmor && !SceneData.SameGender)
				if (packID == 7)
					if (DTSleep_IntimatePoolTableList.HasForm(baseBedForm))
						; big so allow other animations, too
						sidArray.Add(56)  ; for pool table
			...
			elseIf (cuddlesOnly)
				...
				elseIf (!standOnly)
					if (playerPick)
						...
						sidArray.Add(1)		; bed cuddle replace missionary-2
						if (SceneData.SameGender && SceneData.MaleRoleGender == 1 && !DoesSecondActorHateID(4))
							...
							if (includeHugs)
								sidArray.Add(4) ; to give more chance
							endIf
						endIf
						;if (!SceneData.SameGender)
						;	sidArray.Add(46)
					...
					; v1.63 - new Atomic Lust 2.4.2
					if ((DTSConditionals as DTSleep_Conditionals).AtomicLustVers >= 2.42)
						okayAdd = true
						if (playerPick && !DoesMainActorPrefID(5) && !DoesMainActorPrefID(0))
							okayAdd = false
						endIf
						if (okayAdd && (RestrictScenesToErectAngle == 1 || RestrictScenesToErectAngle < 0))
							if (!SceneData.SameGender)
								if (bedIsDouble)
			...
			elseIf (!cuddlesOnly && !SceneData.IsUsingCreature)
				...
				elseIf (!SceneData.SameGender && SceneData.SecondFemaleRole == None && SceneData.SecondMaleRole == None)
					...
					if (!mainActorIsMaleRole)
						...
						numToAdd = (DT_RandomQuestP as DT_RandomQuestScript).GetNextSizedPackIntPublic() - 3
						if (numToAdd < 0)
							numToAdd = 1
						elseIf (numToAdd < 3)
							numToAdd = 2
...
bool Function SceneIDAtPlayerPosition(int sid)
	...
	elseIf (sid >= 546 && sid <= 549)
		return true
	;elseIf (sid == 551)	; not enough room	
	;	return true
