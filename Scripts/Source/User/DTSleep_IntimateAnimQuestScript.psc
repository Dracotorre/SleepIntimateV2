...
int ScenePrefIsProne = 511 const
...
Function SetActorScenePlayerPrefProne()						; v3.25
	MainActorScenePrefArray.Add(ScenePrefIsProne)
endFunction
		...
		if (SceneData.MaleRole != None && SceneData.FemaleRole != None)
			...
			if (SceneData.SameGender)
				...
				elseIf (SceneData.MaleRoleGender == 1)
					...
					; v3.22 moved to another function
...
; returns 1 for FM oral, 2 for FF-only oral, 3 for anal, 4 for both anal and FM-oral, 5 for female-stimulation or spanking (FM or FF), 6 for both female-stimulation and anal
int Function HasFurnitureOralAnalChoiceInt(ObjectReference obj, Form baseBedForm)
	...
	if (obj != None)
		...
		if ((DTSConditionals as DTSleep_Conditionals).IsSavageCabbageActive)
			...
			if (obj.HasKeyword(AnimFurnCouchKY))
				...
				elseIf ((DTSConditionals as DTSleep_Conditionals).IsRZSexActive && (DTSConditionals as DTSleep_Conditionals).RZSexVers >= 2.50)
					; has both v3.20 oral and anal
					; first check if has RZSex  - v3.25
					if ((DTSConditionals as DTSleep_Conditionals).RZSexVers >= 3.00)
						; v3.28 updated for more animations
						if (!GetIsOnIgnoreListSceneID(1142))
							; female stimulation and anal
							return 6
						elseIf ((DTSConditionals as DTSleep_Conditionals).RZSexVers >= 3.50 && !GetIsOnIgnoreListSceneID(1143))
							return 1
						endIf
					endIf
			...
			if (DTSleep_IntimateCouchList.HasForm(baseBedForm))			; v3.15 removed 781 here since must have KY
				...
				if (GetIsOnIgnoreListSceneID(735))
					...
					; first check if has RZSex  - v3.25
					if ((DTSConditionals as DTSleep_Conditionals).IsRZSexActive && (DTSConditionals as DTSleep_Conditionals).RZSexVers >= 3.00 && !GetIsOnIgnoreListSceneID(1142))
						return 5
					endIf
		...
		if ((DTSConditionals as DTSleep_Conditionals).IsRZSexActive)			; v3.15 
			...
			bool isCouch = obj.HasKeyword(AnimFurnCouchKY)
			bool onCouchList = false 
			
			; sceneID 1142 may be any couch, so check list   -- v3.25
			if (baseBedForm != None && !isCouch)
				onCouchList = DTSleep_IntimateCouchList.HasForm(baseBedForm)
			endIf
			
			if (isCouch || onCouchList)
			
				if ((DTSConditionals as DTSleep_Conditionals).RZSexVers >= 3.00 && !GetIsOnIgnoreListSceneID(1142))
					; v3.25 -- female stimulation
					if (!GetIsOnIgnoreListSceneID(1140))
						return 6
					endIf
			...
			while (attemptCount > 0)
				...
				if (SecondActorRef != None && id != 740)
					...
					if (myMainAtMeNode != None && myMainAtMeNode.Is3DLoaded())
						...
						elseIf (id >= 900 && id < 1300)					; adjust limit for future
						
							zOffset = 0.0
							
							; TODO: BP70 position
							if (MainActorPositionByCaller)					; v3.25
								placeOnBedSimple = false
							elseIf (standOnly && onFloorOK)					; v3.15
								placeOnBedSimple = false
							else
								placeOnBedSimple = true
								matchRotation = true
								...
								elseIf (id == 1141)							;v3.21
									...
									headingAngle = SleepBedRef.GetAngleZ()
								elseIf (id == 1142)
									; rotate 180  and shift                     v3.25
									xOffset = 20.0
									yOffset = -8.0
									matchRotation = true
									revHeading = true 
								elseIf (id == 1143)
									; need to rotate 180  v3.28
									matchRotation = true
									revHeading = true 
...
float Function PositionMarkerOnBedZAdjustForSceneID(int id)
	...
	elseIf (id >= 1100 && id <= 1049)
		...
		return 43.25
	elseIf (id >= 1200 && id < 1300)
		; TBOS floor animations on bed   v3.25
		return 43.25
	elseIf (id >= 1000 && id < 1019)					
		; BP70 floor animations on bed
		if (id == 1010)
	...
	if (packID > 0 && packID != 4 && DTSleep_AdultContentOn.GetValue() >= 2.0 && includeHugs <= 1)
			...
			elseIf (!SceneData.CompanionInPowerArmor && SceneData.IsCreatureType != 3 && SceneData.IsCreatureType < 5)
				...
				elseIf (packID == 7)
					...
					elseIf (MySleepBedFurnType == FurnTypeIsSeatSofa)
						...
						if (SceneData.SameGender)
							;sidArray.Add(40)				;v2.73 twerk-dance should not be here		
						elseIf (SceneData.SecondFemaleRole == None && !playerPick)
							if ((DTSConditionals as DTSleep_Conditionals).SavageCabbageVers >= 1.10 && SceneData.SecondMaleRole == None)
								if (baseBedForm == None)
									baseBedForm = SleepBedRef.GetBaseObject() as Form
								endIf
					...
					elseIf (MySleepBedFurnType == FurnTypeIsSeatThrone && !SceneData.SameGender)
						if (playerPick && DoesMainActorPrefID(ScenePrefIsOral))				; v2.70 added oral pick-only
							sidArray.Add(33)
						elseIf (!playerPick && (DTSConditionals as DTSleep_Conditionals).SavageCabbageVers >= 1.20)
							sidArray.Add(87)
						endIf
					endIf
				elseIf (packID == 10)			
				...
				elseIf (packID == 11)			; v3.15
					...
					if (playerPick) 
						if (DoesMainActorPrefID(ScenePrefIsOral))
							...
							if (MySleepBedFurnType == FurnTypeIsSeatSofa && SceneData.SecondMaleRole == None)
								int numAdded = 0
								if ((DTSConditionals as DTSleep_Conditionals).RZSexVers >= 3.50 && !SceneData.SameGender)
									; no same-gender
									sidArray.Add(43)		; v3.28
									numAdded += 1
								endIf
								if (numAdded <= 0 || Utility.RandomInt(0, 6) > 3)
									; limit if added v3.28
									if ((DTSConditionals as DTSleep_Conditionals).RZSexVers >= 3.00)
										; okay for same-gender female or male
										sidArray.Add(42)		; added v3.25
									endIf
								endIf
						...
						elseIf (DoesMainActorPrefID(ScenePrefIsSpank))
							...
							bool sFound = false		; limit to single scene
							...
							if (!sFound && !MainActorPositionByCaller && (DTSConditionals as DTSleep_Conditionals).RZSexVers >= 3.31)
					...
					elseIf (MySleepBedFurnType == FurnTypeIsSeatHigh)
						if (!SceneData.SameGender)
							...
							endIf
						endIf
					elseIf (MySleepBedFurnType == FurnTypeIsSeatStool && (DTSConditionals as DTSleep_Conditionals).RZSexVers >= 3.50)		; v3.28
						if (!SceneData.SameGender)
							sidArray.Add(60)
						endIf
					endIf
		...
		elseIf (MySleepBedFurnType >= 250 && MySleepBedFurnType < 300 && !MainActorPositionByCaller)
			...
			elseIf (!SceneData.IsUsingCreature && !SceneData.CompanionInPowerArmor)
				...
				elseIf (packID == 11)								; v3.15
					if (!SceneData.SameGender && !playerPick)
						if (MySleepBedFurnType == FurnTypeIsTablePicnic)
							;sidArray.Add(4)
							sidArray.Add(6)
				...
				elseIf (packID == 10)		
					if (MySleepBedFurnType == FurnTypeIsTablePicnic && !playerPick)
					...
					if (!SceneData.SameGender)
						if (MySleepBedFurnType == FurnTypeIsTablePool)
							...
							if (SceneData.SecondMaleRole != None)			; v2.53 restrict -- oral choice above
								...
								sidArray.Add(16)	; pool table last half of original 756 -- added v3.28
								if (Utility.RandomInt(0, 6) > 4)
									sidArray.Add(57)  ; for pool table
								endIf
							endIf
						elseIf (MySleepBedFurnType == FurnTypeIsTablePicnic && !playerPick)
							sidArray.Add(43)
						...
						if ((DTSConditionals as DTSleep_Conditionals).IsRufgtActive)
							if (playerPick && DoesMainActorPrefID(ScenePrefIsOral))
								if (SceneData.MaleRoleGender == 1)
									...
									sidArray.Add(92)
								endIf
							elseIf (!playerPick && SceneData.MaleRoleGender == 1)
								sidArray.Add(91)
							endIf
						endIf
		...
		elseIf (packID == 12 && SceneData.SameGender && SceneData.MaleRoleGender == 0 && !SceneData.CompanionInPowerArmor && !SceneData.IsUsingCreature)
		
			if (playerPick && DoesMainActorPrefID(ScenePrefIsOral))
				sidArray.Add(49)
			elseIf (playerPick && DoesMainActorPrefID(ScenePrefIsMission))
				sidArray.Add(1)
			elseIf (playerPick && DoesMainActorPrefID(ScenePrefIsCowboy))
				sidArray.Add(6)
			elseIf (!playerPick)
				sidArray.Add(1)
				sidArray.Add(6)
		...
		elseIf (packID == 11 && !SceneData.IsUsingCreature)
			...
			elseIf (!SceneData.SameGender || SceneData.HasToyAvailable)
				...
				elseIf (!playerPick || DoesMainActorPrefID(ScenePrefIsProne))
					
					sidArray.Add(16)
					if (MySleepBedFurnType == FurnTypeIsDoubleBed && (DTSConditionals as DTSleep_Conditionals).RZSexVers >= 3.00)
						sidArray.Add(17)		; v3.25
					endIf
				endIf
			endIf
...
bool Function SceneIDIsSpanking(int id)				; v3.15
	...
	elseIf (id == 1158)		; v3.27
		return true
	elseIf (id == 947)		; corrected v3.28
		return true
	elseIf (id == 547)
		return true
	elseIf (id == 1020)		; fix-v3.28
		return true
	endIf
	return false
endFunction
...
Function MoveActorsToAACPositions(Actor mainActor, Actor secondActor, Actor thirdActor = None, float mainYOff = 0.0, float mainAngleOff = 0.0, float secondAngleOff = 0.0, float mainZOff = 0.0)
	...
	elseIf (secondActor != None)
		...
		if (mainActor == MainActorRef && mainAngleOff != 0.0)
			; prefer to turn companion instead of main     v3.28
			secondAngleOff += mainAngleOff
			mainAngleOff = 0.0
		endIf
...
bool Function SceneOkayToClonePlayer(int sid)
	...
	elseIf (cloneOkay == 1)
		...
		elseIf (MySleepBedFurnType == FurnTypeIsTablePool)
			...
			if (sid == 756 || sid == 716 || sid == 757)				; v3.28 - added 716 and 757
				return true							; v2.90
			endIf
			return false
		elseIf (MySleepBedFurnType == FurnTypeIsSMBed)
