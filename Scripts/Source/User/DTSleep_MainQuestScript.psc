		...
		if (mainActor != None && mainActor == PlayerRef)
			...
			if ((DTSleep_IntimateAnimQuestP as DTSleep_IntimateAnimQuestScript).FadeEnable)
				...
				if (bedRef != None && SceneData.AnimationSet != 8 && SceneData.Interrupted <= 0)
					...
					if (SceneData.AnimationSet >= 5 && SceneData.AnimationSet < 10 && (IsAAFReady() || hasAAFBeenDisabled))
				...
				if (myCompanion != None)
					...
					elseIf (myCompanion.GetValue(CA_AffinityAV) >= 1000.0)
						if (DTSleep_CompanionRomanceList.HasForm(companionActor as Form))
							return companionActor
						elseIf (DTSleep_SettingIntimate.GetValue() >= 2.0 && PlayerHasPerkOfCompanion(companionActor))
							return companionActor
						endIf
					endIf
				endIf
			...
			if (aCompanion != None && aCompanion != StrongCompanionRef && aCompanion.GetValue(CA_AffinityAV) >= 1000.0 && aCompanion.GetDistance(PlayerRef) < 350.0)
				...
				if (DTSleep_CommonF.IsActorOnBed(aCompanion, bedRef))
					...
					if (DTSleep_CompanionRomanceList.HasForm(aCompanion as Form))
						return aCompanion
					elseIf (DTSleep_SettingIntimate.GetValue() >= 2.0 && PlayerHasPerkOfCompanion(aCompanion))
						return aCompanion
					endIf
		...
		if (PlayerHasActiveCompanion.GetValueInt() > 0 && CompanionAlias != None)
			...
			if (aCompanion != None && aCompanion.IsEnabled() && !aCompanion.IsDead() && !aCompanion.IsUnconscious())
				...
				if (aCompanion.IsInfatuated() || aCompanion.IsRomantic() || DTSleep_CompanionRomanceList.HasForm((aCompanion as Actor) as Form))
					...
					elseIf (!isCompRomantic && (DTSConditionals as DTSleep_Conditionals).IsCoastDLCActive && aCompanion == (DTSConditionals as DTSleep_Conditionals).FarHarborDLCLongfellowRef)
						;DTDebug("GetCompanion Alias Romance-able - skip check Extra-NPC " + aCompanion, 2)
...
Function HandlePlayerActivateBed(ObjectReference targetRef, bool isNaked, bool isSpecialAnimBed)
	...
	if (!cancledScene && !undressCheck && DTSleep_SettingIntimate.GetValue() > 0.0 && companionReady && IntimateCompanionRef != None)
		...
		if (basebedForm != None && !DTSleep_BedNoIntimateList.HasForm(basebedForm))
			...
			if (bedHeight < bedHtLim && bedHeight > -50.0) ; bed should be relatively level with player
				...
				;
				if (DTSleep_AdultContentOn.GetValue() >= 2.0 && DTSleep_SettingLover2.GetValue() >= 1.0 && !SceneData.CompanionInPowerArmor && Debug.GetPlatformName() as bool)
			...
			if (doScene)
				...
				elseIf (animPacks.Length > 0 && (DTSleep_IntimateAnimQuestP as DTSleep_IntimateAnimQuestScript).GetTimeForPlayID(6001) != 224.16)
				...
				int undressLevel = -1
				if (doFade)
					if (DTSleep_AdultContentOn.GetValue() <= 1.5 || SceneData.AnimationSet == 0 || pickStyle == 1)
						undressLevel = 1
				...
				if (sleepTime && playerNakedOrPJ && sleepNapPrefVal > 0 && DTSleep_SettingFadeEndScene.GetValue() >= 1.0)
					if (SceneData.AnimationSet < 5 || SceneData.AnimationSet == 10 || !IsAAFReady())
	...
	elseIf (!cancledScene && !undressCheck && DTSleep_SettingTestMode.GetValueInt() >= 1 && DTSleep_DebugMode.GetValue() >= 2.0 && DTSleep_SettingIntimate.GetValue() > 0.0 && DTSleep_AdultContentOn.GetValue() >= 2.0 && TestVersion == -5 && (DTSConditionals as DTSleep_Conditionals).ImaPCMod)
		...
		if (nearCompanion == None || nearCompanion.CompanionActor == None)
			...
			IntimateLocationHourSet locHourSet = ChanceForIntimateSceneByLocationHour(targetRef, baseBedForm, none, 4, gameTime)
			
			if (locHourSet.LocChanceType == LocActorChanceOwned || locHourSet.NearActors < 3)
			
				int gender = (DTSleep_IntimateUndressQuestP as DTSleep_IntimateUndressQuestScript).GetGenderForActor(PlayerRef)
				
				if (IsAdultAnimationAvailable(gender))
					
					float curTime = Utility.GetCurrentGameTime()
					float hoursSinceSleep = (DTSleep_HealthRecoverQuestP as DTSleep_HealthRecoverQuestScript).GetHoursSinceLastSleepTime(curTime)
					int sleepHours = (DTSleep_HealthRecoverQuestP as DTSleep_HealthRecoverQuestScript).HourCount
					bool isSleepy = (DTSleep_HealthRecoverQuestP as DTSleep_HealthRecoverQuestScript).SleepyPlayer
					
					if (!isSleepy || sleepNapPrefVal <= 0 || hoursSinceSleep > 2.0)
						DTDebug("self-pleasure recheck Sleepy status", 2)
						bool recordCheck = false
						if (hoursSinceSleep > 8.0 || sleepHours < 2)
							recordCheck = true
						endIf
						isSleepy = (DTSleep_HealthRecoverQuestP as DTSleep_HealthRecoverQuestScript).IsSleepTimePublic(curTime, recordCheck)
					endIf
					
					bool isPleasureReady = false
					
					if (hoursSinceSleep < 2.0 && sleepHours >= 2 && sleepHours < 6)
						float hoursSinceLast = DTSleep_CommonF.GetGameTimeHoursDifference(IntimateLastTime, curTime)
						if (hoursSinceLast > 16.0)
							isPleasureReady = true
						endIf
					endIf
					
					DTDebug("self-pleasure check isPleasureReady? " + isPleasureReady + ", sleepHours: " + sleepHours + " isSleepy: " + isSleepy + "--time: " + curTime, 1)  ;TODO
					
					if (isSleepy && isPleasureReady)
						; no prompt, just go
						if (DTSleep_SettingDogRestrain.GetValue() > 0.0)
						...
						if (SetUndressPlaySoloScene(targetRef, gender) >= 1)
							noPreBedAnim = false
							
							IntimateSucPrevTime = IntimateLastTime
							IntimateLastTime = curTime
							DTSleep_IntimateTime.SetValue(curTime)
							;if (IntimacySelfCount < 9999)
							;	IntimacySelfCount += 1
							;endIf
						else
							noPreBedAnim = true
...
Function HandlePlayerActivateFurniture(ObjectReference akFurniture, int specialFurn, bool isNaked = false)
	...
	elseIf (specialFurn > 0 && DTSleep_SettingUndress.GetValue() <= 0.0)	; v2.24 undress check
		hugsOnly = true
		doOtherProp = false
	...
	if (!doSexyDance && !doOtherProp && akFurniture.IsFurnitureInUse())
		...
		elseIf (nearCompanion.CompanionActor == StrongCompanionRef || nearCompanion.CompanionActor.HasKeyword(ActorTypeRobotKY) || nearCompanion.CompanionActor.HasKeyword(ActorTypeSuperMutantBehemothKY))
			...
			if (!isPillory && specialFurn != 4 && specialFurn < 102)
				...
				if (DTSleep_AdultContentOn.GetValue() >= 2.0 && (DTSConditionals as DTSleep_Conditionals).ImaPCMod && TestVersion == -2)
					...
					if (setPickSpot || Utility.RandomInt(2,9) > 6)
	...
	if (companionCompatible)
		...
		readyVal = IsCompanionActorReadyForScene(nearCompanion, false, None, powerArmorOK, hugOK)			; also updates scene-data
	...
	if (companionReady && IntimateCompanionRef != None && furnObjReady)
		...
		;   but swap after passing check!
		;
		if (!hugsOnly && !doDance && specialFurn != 4 && DTSleep_AdultContentOn.GetValue() >= 2.0 && DTSleep_SettingLover2.GetValue() >= 1.0 && !SceneData.CompanionInPowerArmor && Debug.GetPlatformName() as bool)
	...
	if (compActor != None)
		...
		if (playerGender == companionGender)
			...
			if (playerGender == 1)
				...
				if (DTSleep_AdultContentOn.GetValueInt() >= 2 && !inPowerArmor && !hugOkay)
...
bool Function PlayerIntimateCompatible()
	...
	DTDebug("player race incompatible!", 1)
...
Function SetSceneForAltFemBody()
...
int Function SetUndressPlaySexyDance(ObjectReference furnToPlayObj)
	...
	if (furnToPlayObj != None)
		...
		if ((DTSConditionals as DTSleep_Conditionals).SavageCabbageVers >= 1.1 && DTSleep_AdultContentOn.GetValue() >= 2.0 && (DTSConditionals as DTSleep_Conditionals).ImaPCMod)
			if (TestVersion == -2)
				adultReady = true
			endIf
		endIf
		...
		if (IntimateCompanionRef == None || !adultReady)
			...
			if (furnIsPole && (DTSleep_IntimateAnimQuestP as DTSleep_IntimateAnimQuestScript).PlayActionDancePole())
				...
				return 1
			elseIf ((DTSleep_IntimateAnimQuestP as DTSleep_IntimateAnimQuestScript).PlayActionDanceSexy())
				if (SceneData.MaleRole != PlayerRef)
					RegisterForRemoteEvent(furnToPlayObj, "OnActivate")			; watch for NPC activate
				endIf
				RegisterForCustomEvent((DTSleep_IntimateAnimQuestP as DTSleep_IntimateAnimQuestScript), "IntimateSequenceDoneEvent")
...
int Function SetUndressPlaySoloScene(ObjectReference furnToPlayObj, int gender)
	DisablePlayerControlsSleep(1)
	...
	if (furnToPlayObj != None)
		...
		if (!adultReady)
			...
			SetUndressForManualStop(true, None, false, true, furnToPlayObj, includePipboy, true, true)
			
			;if (!(DTSleep_IntimateUndressQuestP as DTSleep_IntimateUndressQuestScript).PlacedSleepwearAtFeetPlayer)
			
			if ((DTSleep_IntimateAnimQuestP as DTSleep_IntimateAnimQuestScript).PlayActionIntimateSoloSeq(gender))
...
int Function SetUndressAndFadeForIntimateScene(Actor companionRef, ObjectReference bedRef, int fadeUndressLevel, bool mainActorIsPositioned, bool playSlowMo = false, bool isDogmeatScene = false, bool lowCam = true, int[] animPacks = None, bool isNaked = false, ObjectReference twinBedRef = None, int playerScenePick = -1)
	...
	if (adultContentVal >= 1 && (DTSConditionals as DTSleep_Conditionals).ImaPCMod)
		...
		if (animPacks.Length > 0)
			...
			if (arousalAngle >= 0 && SceneData.MaleRole == PlayerRef && playerScenePick < 0)
	...
	if (fadeUndressLevel >= 0 && DTSleep_SettingUndress.GetValue() > 0.0)
		...
		if (!isNaked)						; v1.57 - if naked on activate bed, take it all off
			...
			if (adultContentVal < 2 || SceneData.AnimationSet == 0 || animPacks == None)
		...
		if (bedRef != None && (DTSleep_IntimateAnimQuestP as DTSleep_IntimateAnimQuestScript).IsObjSedan(bedRef))
			doPlacePack = false
		endIf
		...
		if (fadeUndressLevel >= 0 && fadeUndressLevel <= 1)
			...
			elseIf ((DTSleep_IntimateUndressQuestP as DTSleep_IntimateUndressQuestScript).StartForManualStopSleepwear(companionRef, bedRef) == false)
	...
	while (i >= 0)
		...
		if (act != None)
			...
			elseIf (nwBarbActor != None && act == nwBarbActor)
				...
				DTSleep_CompanionRomanceList.RemoveAddedForm(act as Form)
			elseIf (act == (DTSConditionals as DTSleep_Conditionals).FarHarborDLCLongfellowRef)
